...
describe("Action", () => {
  ...
  test("action builders can be appended to other action builders", async () => {
    const simple = Action.create({
      id: "first" as const,
      run: (value: string) => Promise.resolve(parseInt(value)),
    }).thenStep({ id: "second" as const, run: value => Promise.resolve(value - 3) })

    const append = Action.create({ id: "next" as const, run: (value: number) => Promise.resolve(value * 2) }).thenStep({
      id: "one-more" as const,
  ...
  test("around works for 3 step Action", async () => {
    const action = Action.create({ id: "test1" as const, run: (value: number) => Promise.resolve(value * 2) })
  ...
  test("action works for some stages", async () => {
    ...
    const exec = Action.create({ id: "s1", run: () => promise1.promise() })
