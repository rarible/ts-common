...
export class Action<Id, In, Out> extends CallableInstance<[In], Promise<Out>> {
  ...
  thenStep<NewId, NewOut>(step: Step<NewId, Out, NewOut>) {
    return new Action<Id | NewId, In, NewOut>([...this.steps, step as Step<NewId, unknown, NewOut>])
  }

  thenAction<NewId, NewOut>(action: Action<NewId, Out, NewOut>) {
    return new Action<Id | NewId, In, NewOut>([...this.steps, ...(action.steps as Step<unknown, unknown, NewOut>[])])
  }
